...
> Позволяет откатить изменения на какую-либо метку (хеш)
```bash
git reset --hard <hash>  # Вернуться на определённый коммит в истории. Все изменения,
# сделанные после этого коммита пропадут.
git reset <hash>  # Вернуться на определённый коммит в истории. Все изменения, сделанные
# после этого коммита, получат состояние "No staged for commit".
# Чтобы вернуть их обратно, нужно изспользовать команды git add и git commit.
git reset --soft <hash>  # Вернуться на определённый коммит в истории. Все изменения,
# сделанные после этого коммита, получать состояние "Staged for commit". Чтобы 
# вернуть их обратно, нужно использовать команду git commit.
```
...
<h2 id="copy-files-from-branches">
Скопировать файлы с другой ветки
</h2>
